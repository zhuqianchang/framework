...
下图是QQ邮箱登录的页面，右上角能够进行语言的切换，这个操作就是国际化。如下：

<div align="left">
    <img src="https://user-gold-cdn.xitu.io/2020/5/25/1724b5de4fcc6f2b?w=770&h=548&f=png&s=16035" alt="国际化"/>
</div>

国际化的目的是使同一个项目能够在不同地区运行。最核心的就是页面上的文字能够满足不同地区的要求。
...
### 添加国际化文件
要想实现国际化，首先需要添加国际化文件。默认的文件名为`messages*.properties`，一般存放的文件路径为：/resources/i18n/，如下：

<div align="left">
    <img src="https://user-gold-cdn.xitu.io/2020/5/25/1724b5f7b41305b1?w=337&h=180&f=png&s=7535" alt="国际化文件"/>
</div>

在文件中以key-value的形式存储数据。
...
需要实现ResourceBundleMessageSource，并指定国际化文件的地址，如下：
```java
@Slf4j
@Configuration
...
程序员可不是只会Ctrl+C和Ctrl+V的，我们可以对返回结果进行统一处理来解决这个问题。

### 统一处理返回结果
每个结果的返回结果都是ApiResult，那我们只要把该返回值拦截到，然后统一进行国际化即可。

通过注解@ControllerAdvice和实现ResponseBodyAdvice接口，
数据返回前都调用beforeBodyWrite方法，在这里我们将code转换成国际化数据。如下：
